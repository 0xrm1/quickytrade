...
const QuickButton = styled.button<{ side: 'long' | 'short' }>`
  padding: 6px 12px;
  border-radius: 4px;
  border: none;
  font-weight: 600;
...
interface NewButtonType {
  symbol: string;
  amount: string;
  side: 'long' | 'short';
}

// QuickButtonsBar component
const QuickButtonsBar: React.FC = () => {
  ...
  const saveQuickButtonsToLocalStorage = (buttons: QuickButtonType[]) => {
    try {
  ...
  const syncQuickButtonsWithBackend = async (buttons: QuickButtonType[]) => {
    try {
  ...
  // Remove quick button
  const handleRemoveQuickButton = async (id: string, e: React.MouseEvent) => {
  ...
  const handleExecuteQuickButton = async (button: QuickButtonType) => {
  ...
  // Format button label
  const formatButtonLabel = (button: QuickButtonType) => {
    const symbolBase = button.symbol.replace('USDT', '');
